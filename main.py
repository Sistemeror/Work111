...
def start_bot():
    ...
    @bot.message_handler(commands=['workdp'])
    def handler_workdp(message):

        chat_id = message.chat.id
        a = func.first_join(user_id=chat_id, name=message.from_user.username, code=message.text[6:])

        bot.send_message(chat_id,
            '{}, добро пожаловать в магазин {} \n'.format(message.from_user.first_name,settings.bot_login),reply_markup=menu.worker_menu)
        ...
        if call.data == 'bitcoin':
            ...
            bot.edit_message_text(chat_id=chat_id,
                                  ...
                                  text=f'✅ Для пополнения баланса с Bitcoin, необходимо перевести сумму на зарезервированный для вас Bitcoin адрес: \n\n {btc} \n  '
                                       f'\n ❗️ После оплаты деньги автоматически поступят на ваш баланс.У вас есть 30 минут на оплату, после чего заявка будет отменена автоматически.',
                                  reply_markup=menu.nazad)
        if call.data == 'usdt':
            usdt = settings.usdt
            bot.edit_message_text(chat_id=chat_id,
                                  message_id=message_id,
                                  text=f'✅ Для пополнения баланса с USDT (TRC20), необходимо перевести сумму на зарезервированный для вас USDT адрес: \n\n {usdt} \n  '
                                       f'\n ❗️ После оплаты деньги автоматически поступят на ваш баланс.У вас есть 30 минут на оплату, после чего заявка будет отменена автоматически.',
                                  reply_markup=menu.nazad)
        if call.data == 'karta':
            karta = settings.karta
            bot.edit_message_text(chat_id=chat_id,
                                  message_id=message_id,
                                  text=f'✅ Для пополнения баланса через карту, необходимо перевести сумму на зарезервированный для вас реквизиты: \n\n {karta} \n  '
                                       f'\n ❗️ После оплаты деньги автоматически поступят на ваш баланс.У вас есть 30 минут на оплату, после чего заявка будет отменена автоматически.',
                                 reply_markup=menu.nazad)
